...
class _HomePageState extends State<HomePage> {
    ...
    return Scaffold(
                    ...
                    body: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Column(children: _buildUpcomingTasks()),
                          const SizedBox(height: 24),
      ...
      bottomNavigationBar: NavigationBar(
        onDestinationSelected: (int index) {
          if (index == 2) {
            // Add Task button is now at index 2
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => AddTaskPage(onExit: _loadTasks),
              ),
            );
          } else {
            setState(() {
              currentPageIndex =
                  index > 2
                      ? index - 1
                      : index; // Adjust index for other destinations
            });
          }
        },
        selectedIndex:
            currentPageIndex < 2 ? currentPageIndex : currentPageIndex + 1,
  ...
  List<Task> _getUpcomingTasks() {
    ...
    if (currentTask != null) {
      ...
      final currentTaskIndex = futureTasks.indexWhere(
        (task) => task.id == currentTask.id,
      );
      if (currentTaskIndex != -1) {
        // Return up to 3 tasks after the current task
        return futureTasks.sublist(currentTaskIndex + 1).take(3).toList();
      }
    }
  ...
  List<TimeSlot> _getAvailableTimeSlots() {
    ...
    for (var task in todayTasks) {
      ...
      if (taskStartTime.isAfter(currentTime)) {
        ...
        if (currentTime.isAfter(now) || currentTime.isAtSameMomentAs(now)) {
          availableSlots.add(TimeSlot(start: currentTime, end: taskStartTime));
        } else if (now.isBefore(taskStartTime)) {
          // If current time is before now but there's still a gap until next task
          availableSlots.add(TimeSlot(start: now, end: taskStartTime));
        }
      }
    ...
    if (currentTime.isAfter(now) && currentTime.isBefore(endOfDay)) {
      availableSlots.add(TimeSlot(start: currentTime, end: endOfDay));
    }
